...
def test_ausuc_on_edge_cases():
    ...
    assert compute_ausuc(logits, targets, np.ones(num_classes).astype(bool)) == (logits.argmax(axis=1) == targets).mean()
    assert compute_ausuc(logits, targets, np.zeros(num_classes).astype(bool)) == (logits.argmax(axis=1) == targets).mean()
