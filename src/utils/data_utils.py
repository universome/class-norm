...
def split_classes_for_tasks(config: Config, random_seed: int) -> List[List[int]]:
    """
    ...
    classes = np.tile(classes, np.ceil(num_classes_to_use / len(classes)).astype(int))
    classes = np.random.RandomState(seed=random_seed).permutation(classes)[:num_classes_to_use]
...
def compute_class_centroids(dataset: "ImageDataset", total_num_classes: int) -> np.ndarray:
    """
