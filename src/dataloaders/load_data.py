...
def load_data(config: Config, img_target_shape: Tuple[int, int]=None,
    ...
    if config.name == 'CUB':
        ...
        ds_train = cub_embedded.load_dataset(config.dir, resnet_type=config.get('resnet_type', 18), is_train=True)
        ds_test = cub_embedded.load_dataset(config.dir, resnet_type=config.get('resnet_type', 18), is_train=False)
        class_attributes = cub.load_class_attributes(config.dir).astype(np.float32)
