...
#             nn.Linear(self.attrs.shape[1], self.config.head_hid_dim),
#             nn.ReLU(),
#             nn.BatchNorm1d(self.config.head_hid_dim),
#             nn.Linear(self.config.head_hid_dim, self.config.hid_dim),
#             nn.ReLU()
#         )

#     def forward(self, x, output_mask: np.ndarray=None):
...
#         x = normalize(x, scale_value=self.config.scale)
#         protos = normalize(protos, scale_value=self.config.scale)
#         logits = x @ protos.t()
